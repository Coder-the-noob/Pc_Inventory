      ...
      <motion.div variants={fadeUp} initial="hidden" whileInView="show" viewport={{ once: true }} className={`grid gap-8 p-8 lg:grid-cols-2 ${isDark ? glassCard : lightCard}`}>
        <div>
          ...
          <div className={`mt-6 space-y-3 text-sm ${isDark ? "text-slate-200" : "text-slate-700"}`}>
            <p className="flex items-center gap-2"><Ticket className="h-4 w-4 text-cyan-300" /> Token or ticket-based booking flow</p>
            <p className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-violet-300" /> Time-slot reservation and queue management</p>
            <p className="flex items-center gap-2"><Headset className="h-4 w-4 text-emerald-300" /> Agent availability and guided handoff</p>
            <p className="flex items-center gap-2"><Bot className="h-4 w-4 text-cyan-300" /> AI fallback when experts are offline</p>
          </div>
        </div>
        <div className={`rounded-2xl border p-6 ${isDark ? "border-white/10 bg-slate-900/70" : "border-slate-200 bg-slate-50"}`}>
          ...
          <div className="mt-5 space-y-3">
            {["Request Guidance", "Get Token", "Book Slot", "Live Session", "Order Finalization"].map((item) => (
              <div key={item} className={`flex items-center justify-between rounded-xl px-4 py-3 text-sm ${isDark ? "bg-slate-800/80" : "bg-white border border-slate-200"}`}>
                <span>{item}</span>
